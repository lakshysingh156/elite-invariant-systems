...
import { contractChanges, versionsByApi } from "@/data/apis";
import type { Severity } from "@/types";
import { cn } from "@/lib/utils";

export const Route = createFileRoute("/_dashboard/contract")({
  head: () => ({ meta: [{ title: "Contract Intelligence — Invariant." }] }),
  component: Contract,
});
...
function Contract() {
  const versions = versionsByApi["stripe-payments"];
  const [expanded, setExpanded] = useState<string | null>("c1");
  const [filter, setFilter] = useState<Severity | "all">("all");

  const counts = order.reduce(
    (acc, s) => ({ ...acc, [s]: contractChanges.filter((c) => c.severity === s).length }),
    ...
    <>
      ...
      <PageBody className="space-y-5">
        ...
        {order
          ...
          .map((sev) => {
            const items = contractChanges.filter((c) => c.severity === sev);
